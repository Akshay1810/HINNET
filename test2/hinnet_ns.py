...
SWING_FREQ_RANGE = (0.5, 2.0)
STEP_FREQ_RANGE = (1.25, 3.0)
...
class HINNetDataset(Dataset):
    ...
    def __init__(self, file_paths, stride=120, scaler=None):
        ...
        for filepath in self.file_paths:
            ...
            with h5py.File(filepath, "r") as f:
                ...
                for i in range(0, num_samples - WINDOW_SIZE, self.stride):
                    ...
                    if delta_l > 5.0:  # discard outlier window
                        continue
...
class WeightedMSELoss(nn.Module):
    """
    Applies a weight `alpha` to the second element (orientation error)
    of the loss calculation.
    Loss = (pred[:, 0] - target[:, 0])**2 + alpha * (pred[:, 1] - target[:, 1])**2
    """
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        target : (B, 2) – (Δl , Δψ )
        """
        trans_err_sq = (pred[:, 0] - target[:, 0]) ** 2  # Δl error squared
        head_err_sq = (pred[:, 1] - target[:, 1]) ** 2  # Δψ error squared

        # Apply the weight to the heading error
...
if __name__ == "__main__":
    ...
    # 1. First, create a scaler and fit it ONLY on the training data
    print("Fitting StandardScaler on training data...")
    scaler = StandardScaler()
    for train_file in tqdm(train_files):
        with h5py.File(train_file, "r") as f:
            acc, gyro = f["acc"][:], f["gyro"][:]
        # Create a temporary dataframe to process
        df_imu = pd.DataFrame(
            {
                "acc_x": acc[:, 0],
                "acc_y": acc[:, 1],
        ...
        norm_imu_data = roll_pitch_compensation(df_imu)
        peak_ratios = calculate_peak_ratios(norm_imu_data[:, 0])
        features_to_fit = np.hstack((norm_imu_data, peak_ratios))
        scaler.partial_fit(features_to_fit)  # Use partial_fit for large datasets
    print("Scaler fitted.")

    train_dataset = HINNetDataset(file_paths=train_files, stride=1)
    ...
    val_dataset = HINNetDataset(file_paths=val_files, stride=1)
    ...
    if os.path.exists(MODEL_PATH):
        ...
        scheduler = MultiStepLR(optimizer, milestones=[10, 35], gamma=0.1)
        print("\n--- Starting Training ---")
    ...
    if not val_files:
        ...
        for test_file_path in all_files:
